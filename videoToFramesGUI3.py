...
class App:
    def __init__(self):
        ...
        self.btnMode = Button(self.ventana,text="CHANGE TO GRAY MODE",width=74,bg='gray30',fg='white',command=self.mode)
        self.btnMode.pack(side='bottom')

        botones.add('SELECT VIDEO',width=17,bg='gray80',command=self.openFile)
        botones.add('EXTRACT FRAMES',width=17,bg='gray80',command=self.initExtract)
        botones.add('SAVE ON/OFF',width=17,bg='gray80',command=self.savin)
        botones.add('STOP',width=17,bg='gray80',command=self.stop_pro)
        botones.alignbuttons()

        self.ventana.mainloop()

    def savin(self):
        if self.savemod == True:
            self.savemod = False
            self.display.appendtext("SAVE MODE: OFF\n")
        else:
            self.savemod = True
            self.display.appendtext("SAVE MODE: ON\n")
